...
const Sing = () => {
  ...
  // Vocals audio is now managed entirely by useVocalsComparison hook.
  // The hook receives vocalsUrl + isPlaying + currentTime as props and handles
  // its own Audio element, Web Audio routing, and playback sync internally.
  // This eliminates the dual-element problem and fixes the inaudible vocals bug.
  ...
  const handleSeek = useCallback((newTime: number) => {
    ...
    // The hook's vocals audio is synced via currentTime prop — no separate seek needed.
  ...
  // Sync vocals volume into the hook's Web Audio output gain.
  // The hook's analyser is BEFORE this gain, so scoring is unaffected by volume changes.
  useEffect(() => {
    const effectiveVolume = (vocalsEnabled && !isMuted)
      ? (volume / 100) * (vocalsVolume / 100)
      : 0;
    setRefVolume(effectiveVolume);
  }, [volume, isMuted, vocalsEnabled, vocalsVolume, setRefVolume]);
